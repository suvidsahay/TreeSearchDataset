...
OUTPUT_FILE_PHASE2 = os.path.join(root, "results_iterative_phase2.jsonl")
...
def _gen_simplified_question(q: str, a: str, kept_texts: List[str], target_hops: int, chat):
    if chat is None:
        raise RuntimeError("GENERATION LLM must be injected (chat_for_gen is None).")
    try:
        return generate_simplified_question(q, a, kept_texts, target_hops, chat=chat)
    except TypeError as e:
        raise TypeError(
            "generate_simplified_question() must accept `chat` injection."
        ) from e
...
def extract_common_attributes_with_llm(document_text: str, chat_model, num_attributes=5) -> List[str]:
    ...
    "tree_construct/extract_common_attributes.j2",
...
def retrieve_passages_with_bm25(query: str, es_client: Elasticsearch, index_name: str, size: int = 100) -> List[
    ...
    try:
        resp = es_client.msearch(body=request, request_timeout=120)
    except Exception as e:
    ...
    for r in resp.get("responses", []):
        for hit in r.get("hits", {}).get("hits", []):
            ...
            if title:
                txt = fetch_wikipedia_page(title)
                if txt:
                    docs.append((title, txt))
    return docs
...
def evaluate_lexical_diversity_llm(questions_list, chat_for_eval):
    ...
    prompt = render_prompt(
    "tree_construct/evaluate_lexical_diversity_llm.j2",
...
def run_phase_2_top_down(successful_questions: List[QuestionState], chat_for_gen, chat_for_eval, chat_for_ver):
    """
    ...
    for idx, start_q in enumerate(candidates):
        ...
        while len(current_state.passages_used) > 1:
            ...
            new_q_text = _gen_simplified_question(current_state.question, current_state.answer, kept_texts, target_hops, chat=chat_for_gen)
            ...
            verification = verify_question_phase2(kept_texts, cand["question"], cand["answer"], chat_model=chat_for_ver)
    ...
    evaluate_lexical_diversity_llm(phase2_questions_for_diversity, chat_for_eval)
...
def main():
    ...
    for record in tqdm(list(fever_data.values())):
        ...
        for iteration_num in range(K_ITERATIONS):
            ...
            for item in parsed_multihop:
                ...
                while revision_attempts < MAX_REVISIONS and not is_successful:
                    # 1. Structural Check
                    minimal_passages = get_required_passages(item_to_process["question"], item_to_process["answer"], all_passage_tuples, chat_model=chat_for_eval)
                    if not minimal_passages:
                        continue
                    new_hop_count = len({t for t, _ in minimal_passages})
                    
                    if new_hop_count <= len(prev_state.passages_used):
                        naturalness = _eval_naturalness(item_to_process["question"], passage_texts, chat=chat_for_eval)
                        revised_text = _revise_question(passage_texts, item_to_process, minimal_passages, naturalness, chat=chat_for_gen)
                ...
                if is_successful:
                    ...
                    verify_res = _verify_question(passages_text, item_to_process["question"], item_to_process["answer"], chat=chat_for_ver)
                    if not isinstance(verify_res, dict):
                        # pp.warning("Verifier returned None/invalid result. Marking as failed.", indent=4)
                        verify_res = {"requires_all_passages": False, "verification_error": "verifier_returned_none"}
                ...
                if status == "passed":
                    ...
                    if len(new_state.passages_used) - 1 < MAX_CANDIDATE_DOCS:
                         ...
                         score, next_p = find_next_passage_multistage(new_state, used_titles, chat_for_eval)
                         if next_p:
        ...
        evaluate_lexical_diversity_llm(phase1_q_list[:10], chat_for_eval)

        # --- PHASE 2: TOP-DOWN SIMPLIFICATION ---

        run_phase_2_top_down(all_generated_questions_phase1, chat_for_gen=chat_for_gen, chat_for_eval=chat_for_eval, chat_for_ver=chat_for_ver)
