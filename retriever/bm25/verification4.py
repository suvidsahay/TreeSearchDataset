...
def verify_question_v3(documents, question, ground_truth_answer):
    ...
    comparison_prompt = f"""You are a strict answer evaluator. Your task is to compare a "Ground Truth Answer" to four different generated answers. For each generated answer, independently determine if it is a complete and semantically equivalent match to the Ground Truth.

Ground Truth Answer:
"{ground_truth_answer}"

---
Generated Answers to Evaluate:
...
Respond ONLY with a JSON object containing four boolean flags. For each flag, set it to `true` if the corresponding generated answer is a complete and correct match to the Ground Truth. Set it to `false` otherwise.

- "Sufficient_A_passage": Is "Answer A" a complete match to the Ground Truth?
- "Sufficient_B_passage": Is "Answer B" a complete match to the Ground Truth?
- "Sufficient_both_passages": Is "Answer Both" a complete match to the Ground Truth?
- "Sufficient_no_passage": Is "Answer None" a complete match to the Ground Truth?

Your JSON response (e.g., {{"Sufficient_A_passage": false, "Sufficient_B_passage": true, "Sufficient_both_passages": true, "Sufficient_no_passage": false}}):
"""
