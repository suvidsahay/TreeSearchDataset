...
@dataclass
class HistoryNode:
    """Represents a successfully generated question and its parent dependency."""
    id: str  # Unique ID for the question (e.g., Q1, Q2, Q3...)
    parent_ids: List[str]  # List of IDs of nodes this question was built from (Parent Question ID)
    question_text: str
    answer: str
    explanation: str
...
def render_quest_tree_dot(claim: str, history: List[HistoryNode], output_filename: str):
    ...
    using HTML-like labels for rich, multi-line content.
    """
    ...
    def sanitize(text):
        ...
        # Escape characters that break DOT (like double quotes, angle brackets)
        return text.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '')

    # NEW HELPER FUNCTION: ID Sanitizer
    def sanitize_id(title, length=20):
        # 1. Replace spaces/dots/dashes with underscores
        title = title.replace(' ', '_').replace('-', '_').replace('.', '_')
        # 2. Remove all remaining non-alphanumeric/non-underscore characters
        title = re.sub(r'[^a-zA-Z0-9_]', '', title)
        # 3. Truncate and ensure it doesn't start with a number (prefixed with P_)
        return title[:length]

    # --- NEW HELPER FUNCTION: Text Wrapper (retained) ---
    def wrap_text(text, limit=100):
        # ... (wrap_text logic remains the same) ...
        if text is None: return "N/A"
        words = text.split()
        if not words: return ""
        ...
        # Join lines using the HTML line break tag for DOT
    ...
    for node in history:
        ...
        <TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" CELLPADDING="4">
          <TR><TD COLSPAN="2" BGCOLOR="lightblue"><B>Q{node.id} ({node.hop_count} Hops)</B></TD></TR>
        ...
        color = "lightgreen" if node.hop_count > 1 else "lightblue"
        node_definitions.add(
            f'{q_node_id} [label={q_label_html}, shape=box, style="filled, rounded", fillcolor="{color}"];')

        # ------------------------------------
        # B. Define Passage Nodes (P-nodes) and Edges
        # ------------------------------------

        # Passage being added/used (always the last one in the list for non-seed nodes)
        if node.is_seed:
            ...
            # --- APPLIED SANITIZATION FIX HERE (Seed Node) ---
            ...
            # Multi-hop: depends on one parent question and the newly added passage
            ...
            # --- APPLIED SANITIZATION FIX HERE (Multi-hop Node) ---
    ...
    pp.success(f"Tree structure saved in DOT format: {output_filename}. Use Graphviz or Pyvis to visualize.", indent=2)
...
def render_quest_tree_html(claim: str, history: List[HistoryNode], output_filename="quest_tree_interactive.html"):
    ...
    Full text, answer, and explanation are displayed in a tooltip on hover.
    Passage nodes now include a clickable link to the Wikipedia source.
    """

    G = nx.DiGraph()

    # Base URL for Wikipedia (titles must be URL-encoded, often just replacing spaces with underscores)
    ...
    # 2. Define Helper Functions for Node/Edge Creation
    def get_node_details(node: HistoryNode, type: str):
        # Create a detailed HTML string for the Pyvis tooltip (title attribute)
        detail_html = (
        ...
        # Determine the visible label (truncated text)
        visible_label = f"Q{node.id}: {node.question_text[:35]}..."

        return visible_label, detail_html

    # 3. Build Graph

    # Track passages to define them only once
    passage_nodes = {}
    ...
    for node in history:
        ...
        # B. Define Dependencies (Edges & Passage Nodes)

        if node.is_seed:
            # 1-hop seed: depends only on the first passage
            passage_title, passage_text = node.passages_used[0]
            ...
            if p_node_id not in passage_nodes:
                ...
                # Create detailed tooltip for Passage Node
                p_title_html = (
                    f"<b>PASSAGE SOURCE:</b> <a href='{wiki_link}' target='_blank'>{passage_title}</a><hr>"
                    f"<b>Full Text:</b> {passage_text}..."
                )

                G.add_node(p_node_id,
                           title=p_title_html,
                           label=f"Passage: {passage_title[:30]}...",
                           color='bisque',
                           shape='ellipse',
                           # 🌟 ADDED Href for external link 🌟
                           href=wiki_link,
                           # Set physics to false for external nodes to prevent moving when clicked
                           physics=False)

            G.add_edge(p_node_id, q_node_id, label="seed question")

        else:
            # Multi-hop: depends on parent question and new passage
            parent_q_node_id = f"Q_{node.parent_ids[0]}"

            # The New Passage is the secondary dependency
            ...
            if new_passage_id not in passage_nodes:
                ...
                p_title_html = (
                    ...
                    f"<b>Full Text:</b> {new_passage_text}..."
                )

                G.add_node(new_passage_id,
                           title=p_title_html,
                           label=f"Passage: {new_passage_title[:30]}...",
                           color='bisque',
                           shape='ellipse',
                           # 🌟 ADDED Href for external link 🌟
                           href=wiki_link,
                ...
                # Edge 1: Parent Question -> Child Question (main context flow)
            G.add_edge(parent_q_node_id, q_node_id, label="expands context")

            # Edge 2: New Passage -> Child Question (new fact used)
            G.add_edge(new_passage_id, q_node_id, label="new fact source", dashes=True)

    # 4. Save to HTML
    net.from_nx(G)

    # --- Configuration to enable HTML tooltips and physics filter ---
    net.set_options("""
        {
          "interaction": {
            "hover": true,
...
def generate_output(claim: str, expansion_history: List[HistoryNode]):
    html_output_filename = f"quest_tree_{claim.replace(' ', '_')[:20]}.html"
    render_quest_tree_html(claim, expansion_history, html_output_filename)
    dot_output_filename = f"quest_tree_{claim.replace(' ', '_')[:20]}.dot"
