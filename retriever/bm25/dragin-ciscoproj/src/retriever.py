        ...
        **search_engine_kwargs,
        ...
        if engine == 'elasticsearch':
            ...
            self.retriever = EvaluateRetrieval(
                    BM25Search(index_name=index_name, hostname='http://localhost:9200', keys={"title": "title", "body": "txt"}, initialize=False, number_of_shards=1),
        ...
        for qid, query in enumerate(queries):
            ...
            if qid in results:
                for did, (score, text) in results[qid].items():
                    _docids.append(did)
                    # _docs.append(text)
                    doc = self.es.get(index="wiki", id=did)["_source"]
                    full_text = doc.get("txt", "")
                    _docs.append(f"{text}: {full_text}")
                    if len(_docids) >= topk:
                        break
...
def bm25search_search(self, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], top_k: int, *args, **kwargs) -> Dict[str, Dict[str, float]]:
    ...
    #print("[DEBUG] Entered bm25search_search")
    #print(f"[DEBUG] Number of corpus documents: {len(corpus)}")
    ...
    if self.initialize:
        #print("[DEBUG] Initializing and indexing corpus...")
        self.index(corpus)
        #print("[DEBUG] Sleeping for Elasticsearch to index documents...")
        time.sleep(self.sleep_for)
    #else:
    #    ;
    #continue
    #print("[DEBUG] Skipping indexing (initialize=False)")

    #retrieve results from BM25
    query_ids = list(queries.keys())
    queries = [queries[qid] for qid in query_ids]

    queries = [q.replace("Ġ", " ").replace("Äł", "") for q in queries]
    #print("[DEBUG] Sample query IDs:", query_ids[:3])
    print("[DEBUG] Sample queries:", queries[:3])
    ...
    for start_idx in tqdm.trange(0, len(queries), self.batch_size, desc='que', disable=kwargs.get('disable_tqdm', False)):
        ...
        #print(f"[DEBUG] Processing batch from index {start_idx} to {start_idx + self.batch_size}")
        #print(f"[DEBUG] Batch query IDs: {query_ids_batch}")
        #print(f"[DEBUG] Batch queries: {batch_queries}")

        results = self.es.lexical_multisearch(
            texts=queries[start_idx:start_idx+self.batch_size],
            top_hits=top_k)
        #print(f"[DEBUG] Retrieved results for batch: {len(results)} queries")
        for (query_id, hit) in zip(query_ids_batch, results):
            ...
            for corpus_id, score, text in hit['hits']:
                #print(f"[DEBUG] Match - doc_id: {corpus_id}, score: {score}")
                scores[corpus_id] = (score, text)
                final_results[query_id] = scores
    #print(f"[DEBUG] Finished search. Final result keys: {list(final_results.keys())[:3]}")
...
def elasticsearch_lexical_multisearch(self, texts: List[str], top_hits: int, skip: int = 0) -> Dict[str, object]:
        ...
        req_body = {
            ...
            "query": {
                "multi_match": {
                    ...
                    "type": "best_fields",
                    "fields":["txt"],
                    #"fields": [self.title_key, self.title_key],
                    "tie_breaker": 0.5
                    }
                },
    ...
    for resp in res["responses"]:
        ...
        for hit in responses:
            #iprint(hit)
            hits.append((hit["_id"], hit['_score'], hit['_source']['title']))
