...
class BasicGenerator:
    def __init__(self, model_name_or_path):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto",
            load_in_8bit=True,
            torch_dtype=torch.float16,  # Important even in 8bit for better activation
            trust_remote_code="falcon" in model_name_or_path
        )
    ...
    def generate_attn(self, input_text, max_length, solver="max", use_entropy=False, use_logprob=False):
        ...
        # Since we can't get attentions in 8-bit, just dummy 1.0 per token
        attns = [1.0 for _ in tokens]
        ...
        if use_entropy:
            ...
            for score in outputs.scores:
                probs.append(score.softmax(dim=-1).cpu())
            entropies = [-torch.sum(p * torch.log(p + 1e-10), dim=-1) for p in probs]
            entropies = [e[0].item() for e in entropies]
        else:
            entropies = None

        return text, tokens, attns, logprobs, entropies
...
class BasicRAG:
    def __init__(self, args):
        #print("[DEBUG] Generator initialized with model:", self.model_name_or_path)
        #print("[DEBUG] Query formulation:", self.query_formulation)
    ...
    def inference(self, question, demo, case):
        ...
        #print("[DEBUG] Prompt starts with:", prompt[:200])  # limit to 200 chars
...
class AttnWeightRAG(BasicRAG):
    ...
    def keep_real_words(self, prev_text, curr_tokens, curr_hit):
        ...
        with torch.no_grad():
             outputs = self.generator.model(input_ids, output_attentions=True)
        atten_tmp = outputs.attentions[-1][0].cpu()  # move attention tensor to CPU immediately
        del outputs
        ...
        for r in range_:
            ...
            att = torch.zeros(input_length,device = self.generator.model.device)
            for i in range(r[0], r[1] + 1):
                if i == 0:
                    continue
                v = atten_tmp[i-1][:r[0]] # 上一位的
                v = v / v.sum()
                t = torch.zeros(input_length,device=self.generator.model.device)
                t[:r[0]] = v
                att += t
            att /= (r[1] - r[0] + 1)
            # merge token for att
            att = torch.tensor([att[rr[0]:rr[1]+1].sum() for rr in range_],device= self.generator.model.device)
            attns.append(att)
        del atten_tmp
        torch.cuda.empty_cache()
            
        # 计算每个超过阈值的 token 在前文的 attentions
        forward_attns = torch.zeros(len(tokens),device=self.generator.model.device)
        hit_cnt = 0
    ...
    def inference(self, question, demo, case):
        print("[DEBUG] AttnWeightRAG inference start")
        print("[DEBUG] Query formulation:", self.query_formulation)
        ...
        while True:
            ...
            if first_step:
                prompt = case
                print("[DEBUG] First step: no retrieval, using case directly.")
            else:
                prompt = "".join([d["case"] + "\n" for d in demo])
                prompt += "Context:\n"
                for i, doc in enumerate(docs):
            ...
            if prompt.strip() == "":
                print("[ERROR] Empty prompt detected! Breaking inference loop.")
                break
            ...
            if not hallucination:
                ...
                if self.query_formulation == "current":
                    ...
                    retrieve_question = self.fetch_last_n_tokens(forward_all, self.retrieve_keep_top_k)
            ...
            tokens_count = len(self.generator.tokenizer.encode(text))
            del new_text, tokens, attns, logprobs, entropies
            torch.cuda.empty_cache()

            if tokens_count > 2048:
                print("[DEBUG] Early stopping because token count exceeded 2048.")
                break
