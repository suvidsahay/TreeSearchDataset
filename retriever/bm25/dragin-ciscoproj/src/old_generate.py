...
class AttnWeightRAG(BasicRAG):
    ...
    def keep_real_words(self, prev_text, curr_tokens, curr_hit):
        ...
        input_ids = self.generator.tokenizer.encode(all_text, return_tensors="pt").to(device)
        input_length = input_ids.shape[1]
        ...
        for i in range(len(curr_hit)):
            if curr_hit[i] == 1:
                idx = curr_st + i
                if idx < len(attns):
                    forward_attns += attns[idx]
                    hit_cnt += 1
                else:
                    print(f"[WARNING] Skipping attns[{idx}] — out of range (len(attns) = {len(attns)})")

        if hit_cnt > 0:
            forward_attns /= hit_cnt
        else:
            print("[WARNING] No valid attention layers found — using zero vector")
            forward_attns = torch.zeros(len(tokens))
        ...
        for i in range(len(curr_hit)):
            if curr_hit[i] == 1:
                ...
                try:
                    ...
                    print(f"[WARNING] Index {idx} out of bounds for attns (len={len(attns)})")
        ...
        #            hit_cnt += 1
        #        else:
        #            print(f"[WARNING] Skipped attns[{idx}] (out of bounds)")

        #if hit_cnt > 0:
        #    forward_attns /= hit_cnt
        #else:
        #    print("[WARNING] No valid hits found — avoiding division by zero")
        #    forward_attns = torch.zeros(len(tokens))  # or some fallback
    ...
    def inference1(self, question, demo, case):
        text = ""
        docs = []
        ...
        while True:
            ...
            if not hallucination:
                ...
                if self.query_formulation in ["direct", "forward_all"]:
                    retrieve_question = " ".join([question, text, ptext])
                else:
                    raise NotImplementedError(f"Unknown query formulation: {self.query_formulation}")

                docs = self.retrieve(retrieve_question, topk=self.retrieve_topk)
                self.last_retrieved_docs = docs  # ✅ Save context
                ...
                text = " ".join([text.strip(), ptext.strip(), new_text.strip()])

            if len(self.generator.tokenizer.encode(text)) > self.generate_max_length or len(text) <= old_len or "the answer is" in text:
                break
        ...
        return text.strip(), context_str.strip()



    def inference2(self, question, demo, case):
        text = ""
        docs = []
        ...
        while True:
            ...
            if not hallucination:
                ...
                docs = self.retrieve(retrieve_question, topk=self.retrieve_topk)
                self.last_retrieved_docs = docs  # ✅ Store context
                ...
                for i, doc in enumerate(docs):
                    prompt += f"[{i+1}] {doc}\n"
                prompt += "Answer in the same format as before.\n"
                prompt += " ".join([case, text, ptext.strip()])
    ...
    def inference(self, question, demo, case):
        ...
        while True:
            ...
            if not hallucination:
                ...
                docs = self.retrieve(retrieve_question, topk=self.retrieve_topk)
                self.last_retrieved_docs = docs  # ✅ Save retrieved context
                prompt = "SAMPLE CASES: \n"
                prompt +="".join([d["case"]+"\n" for d in demo])
                prompt += "CONTEXT :\n"
                for i, doc in enumerate(docs):
                ...
                prompt += "Answer in the same format as sample cases provided using the context.\n"
