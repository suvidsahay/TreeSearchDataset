import json
from retriever import retrieve_best_passage  # Optional: can be removed if not used
...
# Load 100 entries from filtered FEVER dataset
with open('filtered_fever_with_wiki.jsonl', 'r') as f:
...
for record in tqdm(fever_data):
    ...
    wiki_texts = record.get('wikipedia_texts', {})

    if len(wiki_titles) < 2:
        continue

    doc1 = wiki_texts.get(wiki_titles[0], "")
    doc2 = wiki_texts.get(wiki_titles[1], "")
