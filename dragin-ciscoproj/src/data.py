...
class BaseDataset:
    ...
    def format(self, fewshot: int = 0):
        ...
        } for i in range(min(fewshot, len(self.examplars)))] if fewshot else []
...
class Fever(BaseDataset):
    examplars: List[Dict] = [
        {
            ...
            'cot': "Nikolaj Coster-Waldau's work includes the Fox television series New Amsterdam, and the Fox Broadcasting Company is a television network. Therefore, he has worked with the Fox Broadcasting Company.",
            'answer': 'SUPPORTS',
        },
        {
            'question': "The University of Iloilo is a private university.",
            'cot': "The University of Iloilo is a private university in the Philippines.",
            'answer': 'SUPPORTS',
        },
        {
            'question': "The film \"The Terminator\" was released in 1982.",
            'cot': "The film \"The Terminator\" was released in 1984, not 1982.",
            'answer': 'REFUTES',
        },
        {
            'question': "The song \"See You Again\" was written by Justin Bieber.",
            'cot': "The song \"See You Again\" was written by DJ Frank E, Charlie Puth, Wiz Khalifa, and Andrew Cedar. It was not written by Justin Bieber.",
            'answer': 'REFUTES',
        },
    ]
    demo_input_template = lambda self, ques: f'Question: {ques}\nAnswer:'
    ...
    def __init__(self, data_path: str):
        logger.info(f"Loading Fever from {data_path}")
        dataset = []
        with open(os.path.join(data_path, 'filtered_fever_with_wiki.jsonl'), "r") as fin:
            for line in fin:
...
class DPR(BaseDataset):
    examplars: List[Dict] = [
        # Examples for a DPR-style dataset (Open-domain QA)
        {
            'question': "Who was the first person to walk on the moon?",
            'cot': "The first person to walk on the moon was Neil Armstrong. He did so during the Apollo 11 mission.",
            'answer': "Neil Armstrong",
        },
        {
            'question': "What is the capital of France?",
            'cot': "Paris is the capital and most populous city of France.",
            'answer': "Paris",
        },
         {
            'question': "When was the Battle of Gettysburg fought?",
            'cot': "The Battle of Gettysburg was a Union victory fought from July 1 to July 3, 1863, in and around the town of Gettysburg, Pennsylvania.",
            'answer': "July 1-3, 1863",
        },
        {
            'question': "What is the scientific name for a dog?",
            'cot': "The domestic dog is a domesticated species of mammal that was formerly considered a subspecies of the wolf (Canis lupus), but is now classified as a distinct species, Canis familiaris.",
            'answer': "Canis familiaris",
        },
        {
            'question': "In what year did the Titanic sink?",
            'cot': "The RMS Titanic sank in the early morning hours of April 15, 1912, in the North Atlantic Ocean, four days into the ship's maiden voyage from Southampton to New York City.",
            'answer': "1912",
        },
    ]
    # DPR often uses simple Question: Answer: format
    demo_input_template = lambda self, ques: f'Question: {ques}\nAnswer:'
    test_input_template = lambda self, ques: f'Answer the following question by reasoning step-by-step.\n\nQuestion: {ques}\nAnswer:' # Slightly more complex prompt for test
    output_template = lambda self, cot, ans: f'{cot} So the answer is {ans}.'

    def __init__(self, data_path: str):
        logger.info(f"Loading DPR QA from {data_path}")
        dataset = []
        # We need a file containing Q&A pairs. Assuming a format common for DPR evaluation sets.
        # A common format is JSONL with keys like "question", "answers" (list of strings), "q_id".
        # The passages file (psgs_w100.tsv) is separate and not loaded here directly for evaluation.
        qa_file = os.path.join(data_path, 'dpr-dev-qa.jsonl') # Placeholder filename

        if not os.path.exists(qa_file):
             logger.error(f"DPR QA file not found: {qa_file}. Expected format: JSONL with 'question', 'answers' (list), 'q_id'.")
             logger.info(f"The psgs_w100.tsv file contains passages, not the QA pairs for evaluation.")
             self.dataset = Dataset.from_list([]) # Initialize empty dataset
        ...
        try:
            with open(qa_file, "r", encoding="utf-8") as fin:
                for line in tqdm(fin):
                    try:
                        ...
                        qid = str(example.get("q_id", example.get("id"))) # Use q_id or id
                        question = example.get("question")
                        answers = example.get("answers") # Expecting a list of strings

                        if not qid or not question or not isinstance(answers, list):
                             logger.warning(f"Skipping malformed or incomplete example: {line.strip()}")
                             continue

                        # Ensure all answers are strings
                        answer_list = [str(a) for a in answers if a is not None]


                        dataset.append({
                            'qid': qid,
                            'question': question,
                            'answer': answer_list, # Store as list
                            # 'positive_ctxs': example.get('positive_ctxs', []), # Optional: store positive contexts
                            # 'hard_negative_ctxs': example.get('hard_negative_ctxs', []), # Optional
                        })
            ...
            logger.error(f"Error reading DPR QA file {qa_file}: {e}")
            self.dataset = Dataset.from_list([])
            return


        if not dataset:
             logger.warning("No data loaded for DPR.")

        self.dataset = Dataset.from_list(dataset)
        # Note: psgs_w100.tsv is the passage collection, typically used by a retriever.
        # It's not loaded into the self.dataset here as this class evaluates answer strings.


    def get_real_prediction(self, pred):
        # Implementation for DPR to extract the answer
        # DPR answers are often short spans. Extracting after "So the answer is" is suitable.
        pred_lower = pred.lower()
        answer_prompts = ["the answer is"]
        for prmt in answer_prompts:
            if prmt in pred_lower:
                beg = pred_lower.find(prmt) + len(prmt)
                pred_after_prompt = pred[beg:].strip()

                # Remove common stop words from the end
                stop_words = ["</s>", "<|endoftext|>", "\n", "."]
                for stop_word in stop_words:
                     if pred_after_prompt.endswith(stop_word):
                          pred_after_prompt = pred_after_prompt[:len(pred_after_prompt) - len(stop_word)].strip()
                return pred_after_prompt
        # If no specific prompt found, return the whole prediction (might be wrong)
        return pred.strip()
